...
blogRouter.post('/', async (request, response) => {
    try {
        ...
        if (body.author === "") return response.status(400).json({ error: 'Entry needs an author name' })
...
blogRouter.put('/:id', async (request, response) => {
    ...
    try {
        const updatedEntry = await Blog
            .findOneAndUpdate({ "_id": id }, { $inc: { likes: 1 } }, { new: false })
        if (updatedEntry) {
...
blogRouter.put('/:id/comments', async (request, response) => {
    ...
    try {
        const updatedEntry = await Blog
            .findOneAndUpdate({ "_id": id }, { $push: { comments: request.body.comment } }, { safe: true, upsert: true })
        if (updatedEntry) {
            response.status(201).end()
        } else {
            response.status(500).end()
